...
| medium | code-reviewer ＋ true の観点に対応するサブエージェント | なし |
| high | code-reviewer ＋ true の観点に対応するサブエージェント全部 ＋ coderabbit CLI（並行） | codex / `/code-review ultra` |
...
`migrations` / `security` / `concurrency` には専用サブエージェントが無い。これらは tier 判定（true なら high）に効くが、専門レビューは常時起動の `code-reviewer` が一般観点としてカバーする。high tier では並行する coderabbit CLI もこれらを拾う。専用エージェントが必要になったらこの表に追加する。
...
### coderabbit CLI（high tier のみ）

```bash
coderabbit review --agent -t committed --base <base>
```

- `-t committed` 固定（コミット済みの変更のみレビュー）。`all` は使わない（未コミットの作業ツリーまで巻き込まないため）
